    ...
    def searchForTest(self):
        ...
        for file in allfiles:
            try:
                ...
                print e
                continue
        print("The average overlap num is %f"%(float(totallap)/totalnum))
...
def generateTest(test, featureLoader):
    with open(QUERY_PATH+'single_word.txt', 'r') as queryFile:
        query = queryFile.readline().strip('\n')
        count = 0
        while query and count < 1000:
            try:
                Quires = query.split(',')
                Phones = []
                for quiry in Quires:
